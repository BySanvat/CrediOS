...
export type SmartPaymentState = {
  ok: boolean;
  error?: string;
};

function friendlyPaymentError(caught: unknown) {
  const message = caught instanceof Error ? caught.message : "";
  if (!message) return "No pudimos registrar el pago. Revisa el valor e intentalo de nuevo.";
  if (message.includes("record_installment_payment_with_optional_extra")) {
    return "Falta aplicar la migracion 0005_smart_installment_payment_rpc.sql en Supabase.";
  }
  return message;
}

export async function recordSmartPaymentAction(_state: SmartPaymentState, formData: FormData): Promise<SmartPaymentState> {
  try {
    const ctx = await getAppContext();
    if (!ctx.configured) return { ok: false, error: "Supabase no esta configurado para registrar pagos." };
    ...
    return { ok: false, error: friendlyPaymentError(caught) };
  }
}
